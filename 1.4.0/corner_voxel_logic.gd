...
# --- CORNER DEFINITION (Single Voxel) ---
# Corrected: Occupies only one grid space, just like the wedge/cube.
...
func _ready():
	...
	print("📐 CORNER: Looking for signals on:", parent)
...
func _on_dropped(_by):
	...
	print("📐 CORNER: ✋ RELEASED (Calculating Snap...)")
	var obj = get_parent()
	...
	print("📐 CORNER: ✅ New Grid Positions: ", new_grid_positions)
	...
	if overlap_found:
		print("📐 CORNER: ❌ Placement blocked – space already occupied. Deleting corner piece.")
		obj.queue_free()
		return
	...
	print("📐 CORNER: 🔒 SNAPPED & LOCKED at ", snapped_center)
